// ===================================================================================
//
//                        TAOBAO APP - SELF-CONTAINED MODULE V2
//
// ===================================================================================
...
if (!window.EPhone) {
    ...
    let standaloneBalance = 560.00; // Match your screenshot for testing
    ...
    window.EPhone = {
        ...
        api: {
            ...
            updateWallet: async (amount, description, onUpdateCompleteCallback) => {
                console.log(`[EPhone STUB] Wallet updated by ${amount}. Reason: ${description}`);
                standaloneBalance += amount;
                console.log(`[EPhone STUB] New balance is ${standaloneBalance.toFixed(2)}`);
...
(function(window) {
    ...
    const TaobaoApp = {
        ...
        injectTaobaoStyles: function() {
            ...
            style.textContent = `
                /* CSS content is unchanged, keeping it minimal for brevity */
                .message-bubble.is-cart-share-request .content, .message-bubble.is-gift-notification .content { padding: 0; background: transparent; box-shadow: none; border: none; backdrop-filter: none; -webkit-backdrop-filter: none; }
                .cart-share-card { width: 230px; border-radius: 12px; background-color: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; border: 2px solid transparent; transition: all 0.3s ease; }
                ...
                .gift-card { width: 230px; border-radius: 12px; background-color: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.08); overflow: hidden; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
        ...
        integrateWithHost: function() {
            ...
            } else { console.warn(`TaobaoApp: Placeholder element '#taobao-app-placeholder' not found.`); }
        },
        bindEvents: function() {
            ...
            document.getElementById('taobao-ai-product-results-grid').addEventListener('click', async (e) => {
                if (e.target.classList.contains('add-to-my-page-btn') && !e.target.disabled) {
                    ...
                    e.target.textContent = '已添加'; e.target.disabled = true;
        ...
        renderTaobaoProducts: async function(category = null) {
            ...
            gridEl.innerHTML = '<p>加载中...</p>';
            const allProducts = await this.db.taobaoProducts.orderBy('id').reverse().toArray();
            ...
            gridEl.innerHTML = '';
            if (productsToRender.length === 0) { gridEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); grid-column: 1 / -1; padding: 50px 0;">还没有商品哦，快去添加吧！</p>'; return; }
            productsToRender.forEach(product => {
                const card = document.createElement('div');
                card.className = 'product-card';
                card.dataset.productId = product.id;
                card.innerHTML = `<img src="${product.imageUrl}" class="product-image" alt="${product.name}" onerror="this.src='${this.getRandomDefaultProductImage()}'"><div class="product-info"><div class="product-name">${product.name}</div><div class="product-price">${product.price.toFixed(2)}</div></div><button class="add-cart-btn" data-product-id="${product.id}">+</button>`;
        ...
        renderTaobaoCart: async function() {
            ...
            if (cartItems.length === 0) { listEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); padding: 50px 0;">购物车空空如也~</p>'; checkoutBar.style.display = 'none'; await this.updateCartBadge(); return; }
            ...
            for (const item of cartItems) {
                ...
                if (!product) { await this.db.taobaoCart.delete(item.id); continue; };
                totalItems += item.quantity; totalPrice += product.price * item.quantity;
                const itemEl = document.createElement('div');
                itemEl.className = 'cart-item';
                itemEl.innerHTML = `<img src="${product.imageUrl}" class="product-image" data-product-id="${product.id}" onerror="this.src='${this.getRandomDefaultProductImage()}'"><div class="cart-item-info" data-product-id="${product.id}"><div class="product-name">${product.name}</div><div class="product-price">¥${product.price.toFixed(2)}</div></div><div class="quantity-controls"><button class="quantity-decrease" data-cart-id="${item.id}" ${item.quantity <= 1 ? '' : ''}>-</button><span class="quantity-display">${item.quantity}</span><button class="quantity-increase" data-cart-id="${item.id}">+</button></div><button class="delete-cart-item-btn" data-cart-id="${item.id}">×</button>`;
        ...
        handleAddToCart: async function(productId) {
            ...
            await this.updateCartBadge();
            if (document.getElementById('taobao-cart-view').classList.contains('active')) { await this.renderTaobaoCart(); }
        },
        ...
        openProductDetail: async function(productId) {
            ...
            bodyEl.innerHTML = `<img src="${product.imageUrl}" class="product-image" alt="${product.name}" onerror="this.src='${this.getRandomDefaultProductImage()}'"><h2 class="product-name">${product.name}</h2><p class="product-price">¥${product.price.toFixed(2)}</p>`;
        ...
        handleCheckout: async function() {
            ...
            if (isNaN(totalPrice) || totalPrice <= 0) { await window.EPhone.showCustomAlert('提示', '购物车是空的，无法结算哦！'); return; }
        ...
        renderTaobaoOrders: async function() {
            ...
            for (const order of orders) {
                ...
                itemEl.innerHTML = `<img src="${product.imageUrl}" class="product-image" onerror="this.src='${this.getRandomDefaultProductImage()}'"><div class="order-info"><div class="product-name">${product.name} (x${order.quantity})</div><div class="order-status">${order.status || '已付款'}</div><div class="order-time">${new Date(order.timestamp).toLocaleString()}</div></div>`;
                listEl.appendChild(itemEl);
            }
        },
        clearTaobaoProducts: async function() {
            ...
            if (confirmed) {
                ...
                } catch (error) { console.error("清空桃宝商品时出错:", error); await window.EPhone.showCustomAlert('操作失败', `发生错误: ${error.message}`); }
            }
        },
        handleShareCartRequest: async function() {
            const cartItems = await this.db.taobaoCart.toArray();
            if (cartItems.length === 0) { await window.EPhone.showCustomAlert('提示', '购物车是空的，无法分享哦！'); return; }
            ...
            const message = { role: 'user', type: 'cart_share_request', timestamp: Date.now(), payload: { totalPrice: totalPrice, itemCount: cartItems.reduce((sum, item) => sum + item.quantity, 0) } };
            await window.EPhone.api.sendMessage(targetChatId, message);
            await this.clearTaobaoCart();
            await window.EPhone.showCustomAlert('分享成功', `已向“${char.name}”发送代付请求！`);
            if (this.isRunningInEPhone) { window.EPhone.showScreen('chat-list-screen'); }
        },
        handleBuyForChar: async function() {
            const cartItems = await this.db.taobaoCart.toArray();
            if (cartItems.length === 0) { await window.EPhone.showCustomAlert('提示', '购物车是空的，无法为Ta购买哦！'); return; }
            ...
            const totalPrice = parseFloat(document.getElementById('taobao-checkout-btn').dataset.totalPrice);
            const currentBalance = await window.EPhone.api.getWalletBalance();
            if (currentBalance < totalPrice) { await window.EPhone.showCustomAlert('余额不足!', `为“${char.name}”购买礼物需要 ¥${totalPrice.toFixed(2)}，但你的余额只有 ¥${currentBalance.toFixed(2)}。`); return; }
            ...
            if (confirmed) {
                const buyCallback = async () => {
                    ...
                    const products = (await Promise.all(productPromises)).filter(p => p);
        ...
        sendGiftNotificationToChar: async function(targetChatId, products, cartItems, totalPrice) {
            ...
            const itemsSummary = products.map((p, i) => { const cartItem = cartItems.find(item => item.productId === p.id); return `${p.name} x${cartItem ? cartItem.quantity : '?'}`; }).join('、');
            const visibleMessage = { role: 'user', type: 'gift_notification', timestamp: Date.now(), payload: { senderName: userNickname || '我', itemSummary: itemsSummary, totalPrice: totalPrice, itemCount: cartItems.length, } };
            const hiddenMessage = { role: 'system', content: `[系统指令：用户刚刚为你购买了${cartItems.length}件商品作为礼物，总价值为${totalPrice.toFixed(2)}元。商品包括：${itemsSummary}。请根据你的人设对此表示感谢或作出其他反应。]`, isHidden: true, timestamp: Date.now() + 1 };
        ...
        saveProduct: async function() {
            ...
            if (!name || isNaN(price) || price < 0) { await window.EPhone.showCustomAlert('错误', '商品名称和价格不能为空，且价格不能为负数！'); return; }
            if (!imageUrl) { imageUrl = this.getRandomDefaultProductImage(); }
            const productData = { name, price, imageUrl, category, reviews: [] };
            try {
                if (this.state.currentEditingProductId) { await this.db.taobaoProducts.update(this.state.currentEditingProductId, productData); await window.EPhone.showCustomAlert('成功', '商品已更新！'); }
                else { await this.db.taobaoProducts.add(productData); await window.EPhone.showCustomAlert('成功', '商品已添加！'); }
                document.getElementById('taobao-product-editor-modal').classList.remove('visible');
                await this.renderTaobaoProducts(this.state.currentTaobaoCategory);
            } catch (error) { await window.EPhone.showCustomAlert('失败', `保存商品时出错: ${error.message}`); }
        },
        generateProductReviews: async function(productId) {
            await window.EPhone.showCustomAlert('AI功能', '正在为商品生成评价...');
            const fakeReviews = [ { author: 'AI买家_1', text: '这件商品真是太棒了，完全超出了我的预期！质量上乘，细节处理得非常好。' }, { author: 'AI买家_2', text: '物流很快，包装也很用心。打开包裹的一瞬间就被惊艳到了，推荐购买！' } ];
            const product = await this.db.taobaoProducts.get(productId);
            if (product) { product.reviews = fakeReviews; await this.db.taobaoProducts.put(product); await this.openProductDetail(productId); }
        },
        handleSearchProductsAI: async function() { await window.EPhone.showCustomAlert('AI功能', '搜索功能正在开发中...'); },
        handleAddFromLink: async function() { await window.EPhone.showCustomAlert('AI功能', '链接识别功能正在开发中...'); },
        handleGenerateProductsAI: async function() { await window.EPhone.showCustomAlert('AI功能', 'AI生成商品功能正在开发中...'); },
        showProductActions: async function(productId) {
            const product = await this.db.taobaoProducts.get(productId);
            if (!product) return;
            const action = await window.EPhone.showCustomPrompt('操作商品', `选择对 "${product.name}" 的操作 (输入 'edit' 或 'delete')`);
            if (action === 'edit') { this.openProductEditor(productId); }
            else if (action === 'delete') {
                const confirmed = await window.EPhone.showCustomConfirm('确认删除', `确定要删除商品 "${product.name}" 吗？`);
                if (confirmed) { await this.db.taobaoProducts.delete(productId); await this.renderTaobaoProducts(this.state.currentTaobaoCategory); await window.EPhone.showCustomAlert('成功', '商品已删除。'); }
            }
        },
        openLogisticsView: async function(orderId) {
             const order = await this.db.taobaoOrders.get(orderId);
             if(!order) { await window.EPhone.showCustomAlert('错误', '找不到该订单'); return; }
             window.EPhone.showScreen('taobao-logistics-screen');
             await this.renderLogisticsView(order);
        },
        renderLogisticsView: async function(order) { await window.EPhone.showCustomAlert('提示', '物流详情功能正在模拟中...'); },
    ...
    function addLongPressListener(element, callback, duration = 500) {
        let pressTimer;
        const startPress = (e) => { e.preventDefault(); pressTimer = window.setTimeout(() => callback(e), duration); };
    ...
    window.TaobaoAppModule = {
        init: function() {
            ...
            TaobaoApp.initInternal().then(() => {
                ...
                console.error("Taobao App initialization failed:", err);
            });
        }
    };
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            if (!TaobaoApp.isInitialized) window.TaobaoAppModule.init();
        });
    } else {
        if (!TaobaoApp.isInitialized) window.TaobaoAppModule.init();
    }
