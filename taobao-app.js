    ...
    window.EPhone = {
        ...
        api: {
            getWalletBalance: async () => {
                return Promise.resolve(standaloneWalletBalance);
            },
            ...
            getChat: (chatId) => {
                 console.log(`Standalone Mode: Getting chat for ${chatId}`);
                 // 返回一个模拟的角色对象，以便测试
                 return Promise.resolve({ id: chatId, name: `模拟角色 ${chatId}` });
            },
            openCharSelector: (title) => {
                console.log(`Standalone Mode: Character selector opened for "${title}"`);
                // 模拟用户选择第一个角色
                return Promise.resolve('char_12345');
            },
...
(function(window) {
    'use strict';
        ...
        // ▼▼▼ 【核心新增：日志工具】 ▼▼▼
        const logInfo = (message, data = '') => console.log(`%c🚀 [桃宝App] ${message}`, 'color: #2980b9; font-weight: bold;', data);
        const logSuccess = (message) => console.log(`%c✅ [桃宝App] ${message}`, 'color: #27ae60;');
        const logError = (message, error) => console.error(`%c❌ [桃宝App] ${message}`, 'color: #c0392b;', error);
        // ▲▲▲ 【新增结束】 ▲▲▲
    ...
    const TaobaoApp = {
        ...
        createTaobaoHTML: function() {
            ...
            htmlWrapper.innerHTML = `
                ...
                <div id="taobao-logistics-screen" class="screen">
                    <div class="header">
                        <span class="back-btn" id="taobao-logistics-back-btn">‹</span>
                        <span>物流详情</span>
                ...
                <div id="taobao-add-from-link-modal" class="modal">
                    <div class="modal-content" style="height: auto;">
                        ...
                        <div class="modal-body"><textarea id="taobao-link-paste-area" rows="6" placeholder="请在这里粘贴完整的淘宝或拼多多分享文案..."></textarea></div>
                        <div class="modal-footer">
                            <button class="cancel" id="taobao-cancel-link-paste-btn">取消</button>
                            <button class="save" id="taobao-confirm-link-paste-btn">识别</button>
                        </div>
                    </div>
                </div>
                <!-- ▼▼▼ AI生成商品结果弹窗 (Modal) ▼▼▼ -->
                <div id="taobao-ai-generated-products-modal" class="modal">
                    <div class="modal-content" style="height: 80%;">
                        <div class="modal-header"><span id="taobao-ai-products-modal-title">AI为你生成了以下宝贝</span></div>
                        <div class="modal-body" style="padding: 15px;"><div id="taobao-ai-product-results-grid" class="product-grid"></div></div>
                        <div class="modal-footer"><button class="save" id="taobao-close-ai-products-modal-btn" style="width: 100%;">完成</button></div>
                    </div>
                </div>
            `;
        ...
        renderTaobaoProducts: async function(category = null) {
            ...
            gridEl.innerHTML = ''; // 清空加载提示
            if (productsToRender.length === 0) {
                gridEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); grid-column: 1 / -1; padding: 50px 0;">还没有商品哦，快去添加吧！</p>';
                return;
            }
            ...
            productsToRender.forEach(product => {
                ...
                card.innerHTML = `
                    <img src="${product.imageUrl}" class="product-image" alt="${product.name}" onerror="this.src='${this.getRandomDefaultProductImage()}'">
                ...
                `;
                // addLongPressListener 是全局函数，可以直接调用
        ...
        renderTaobaoCart: async function() {
            ...
            for (const item of cartItems) {
                ...
                if (!product) {
                    // 如果商品被删除了，顺便从购物车移除
                    await this.db.taobaoCart.delete(item.id);
                    continue;
                };
                ...
                itemEl.innerHTML = `
                    <img src="${product.imageUrl}" class="product-image" data-product-id="${product.id}" onerror="this.src='${this.getRandomDefaultProductImage()}'">
                    <div class="cart-item-info" data-product-id="${product.id}">
                        <div class="product-name">${product.name}</div>
                        <div class="product-price">¥${product.price.toFixed(2)}</div>
                    </div>
                    <div class="quantity-controls">
                        <button class="quantity-decrease" data-cart-id="${item.id}" ${item.quantity <= 1 ? '' : ''}>-</button>
        ...
        handleAddToCart: async function(productId) {
            logInfo('Adding to cart:', productId);
        ...
        handleChangeCartItemQuantity: async function(cartId, change) {
            ...
            if (newQuantity <= 0) {
                await this.handleRemoveFromCart(cartId); // 这里不再二次确认，因为用户是主动点的减号
            } else {
        ...
        handleCheckout: async function() {
            ...
            if (isNaN(totalPrice) || totalPrice <= 0) {
                await window.EPhone.showCustomAlert('提示', '购物车是空的，无法结算哦！');
                return;
            }
    ...
    await window.EPhone.api.updateWallet(-totalPrice, '桃宝购物', checkoutCallback);
        ...
        clearTaobaoProducts: async function() {
            ...
            if (confirmed) {
                try {
                    ...
                    await this.renderTaobaoProducts(); 
                    await this.renderTaobaoCart(); // 确保购物车视图也刷新
                    await this.updateCartBadge();
                    await window.EPhone.showCustomAlert('操作成功', '所有商品及购物车已被清空！');
                } catch (error) {
                    logError("清空桃宝商品时出错:", error);
        ...
        handleShareCartRequest: async function() {
            const cartItems = await this.db.taobaoCart.toArray();
            if (cartItems.length === 0) {
                 await window.EPhone.showCustomAlert('提示', '购物车是空的，无法分享哦！');
                return;
            };
            ...
            if (!char) {
                 logError("无法获取角色信息", {targetChatId});
                 return;
            };
            ...
            if (this.isRunningInEPhone) {
                window.EPhone.showScreen('chat-list-screen'); // 仅在EPhone环境中跳转
            }
        },
        ...
        handleBuyForChar: async function() {
            const cartItems = await this.db.taobaoCart.toArray();
            if (cartItems.length === 0) {
                 await window.EPhone.showCustomAlert('提示', '购物车是空的，无法为Ta购买哦！');
                return;
            }
            ...
            if (confirmed) {
                await window.EPhone.api.updateWallet(-totalPrice, `为 ${char.name} 购买礼物`);
                await this.db.userWalletTransactions.add({
                    amount: -totalPrice,
                    description: `为 ${char.name} 购买礼物`,
                    timestamp: Date.now()
                });
        ...
        sendGiftNotificationToChar: async function(targetChatId, products, cartItems, totalPrice) {
            ...
            const itemsSummary = products.map((p, i) => {
                ...
                return `${p.name} x${cartItem ? cartItem.quantity : '?'}`;
            }).join('、');
            ...
            const hiddenMessage = { 
                role: 'system',
                content: `[系统指令：用户刚刚为你购买了${cartItems.length}件商品作为礼物，总价值为${totalPrice.toFixed(2)}元。商品包括：${itemsSummary}。请根据你的人设对此表示感谢或作出其他反应。]`, 
                isHidden: true,
        ...
        clearTaobaoCart: async function() {
            await this.db.taobaoCart.clear();
            await this.renderTaobaoCart();
            await this.updateCartBadge(); // 确保角标也更新
        },
        ...
        saveProduct: async function() {
            ...
            if (!name || isNaN(price) || price < 0) {
                await window.EPhone.showCustomAlert('错误', '商品名称和价格不能为空，且价格不能为负数！'); 
                return;
            }
            if (!imageUrl) {
            ...
            try {
                if (this.state.currentEditingProductId) {
                    await this.db.taobaoProducts.update(this.state.currentEditingProductId, productData);
                     await window.EPhone.showCustomAlert('成功', '商品已更新！');
                } else {
                    await this.db.taobaoProducts.add(productData);
                     await window.EPhone.showCustomAlert('成功', '商品已添加！');
                }
                document.getElementById('taobao-product-editor-modal').classList.remove('visible');
                await this.renderTaobaoProducts(this.state.currentTaobaoCategory);
            } catch (error) {
                logError('保存商品失败', error);
                await window.EPhone.showCustomAlert('失败', `保存商品时出错: ${error.message}`);
            }
        },
        // ... (其他AI和复杂函数的占位实现)
        generateProductReviews: async function(productId) {
            ...
            // 模拟AI生成
            const fakeReviews = [
                { author: 'AI买家_1', text: '这件商品真是太棒了，完全超出了我的预期！质量上乘，细节处理得非常好。' },
                { author: 'AI买家_2', text: '物流很快，包装也很用心。打开包裹的一瞬间就被惊艳到了，推荐购买！' }
            ];
             const product = await this.db.taobaoProducts.get(productId);
             if (product) {
                 product.reviews = fakeReviews;
                 await this.db.taobaoProducts.put(product);
                 await this.openProductDetail(productId); // 刷新详情弹窗
             }
        },
        handleSearchProductsAI: async function() {
             await window.EPhone.showCustomAlert('AI功能', '搜索功能正在开发中...');
        },
        handleAddFromLink: async function() {
             await window.EPhone.showCustomAlert('AI功能', '链接识别功能正在开发中...');
        },
        handleGenerateProductsAI: async function() {
             await window.EPhone.showCustomAlert('AI功能', 'AI生成商品功能正在开发中...');
        },
        ...
        openLogisticsView: async function(orderId) {
             ...
             if(!order) {
                 await window.EPhone.showCustomAlert('错误', '找不到该订单');
                 return;
             }
         ...
         renderLogisticsView: async function(order) {
            // 此函数模拟物流更新
            // ... (可以从原代码复制粘贴过来并适配)
            await window.EPhone.showCustomAlert('提示', '物流详情功能正在模拟中...');
        },
    };
    ...
    function addLongPressListener(element, callback, duration = 500) {
        ...
        const startPress = (e) => { 
            // 阻止默认的上下文菜单（在桌面端右键时）
            e.preventDefault(); 
            pressTimer = window.setTimeout(() => callback(e), duration); 
        };
    ...
    window.TaobaoAppModule = {
        init: function() {
            // 防止重复初始化
            if (TaobaoApp.isInitialized) {
                logInfo("模块已经初始化，跳过。");
                return;
            };
            
            // 使用 async/await 确保初始化按顺序完成
            (async () => {
                try {
