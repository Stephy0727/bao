    ...
    window.EPhone = {
        ...
        api: {
            ...
            updateWallet: async (amount, description) => {
                ...
                if (window.TaobaoAppModule) {
                   // 2. 等待异步的数据库操作完成
                   await window.TaobaoAppModule.addTransactionForStandalone(amount, description);
                }
                // async 函数会自动返回一个 resolved Promise，所以不需要手动 return
            },
            // ▲▲▲ 【修复结束】 ▲▲▲
            getChat: (chatId) => null,
