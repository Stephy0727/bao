// ===================================================================================
//
//              TAOBAO APP - SELF-CONTAINED MODULE (v1.2 - Fully Restored & Fixed)
//
// ===================================================================================
//  This version is a direct refactoring of the original code provided by the user.
//  It preserves all original HTML and CSS, fixes the white screen, non-clickable UI,
//  and the wallet balance refresh issue.
// ===================================================================================
...
if (!window.EPhone) {
    ...
    window.EPhone = {
        ...
        api: {
            ...
            getChat: (chatId) => Promise.resolve(null),
...
(function(window) {
    ...
    const logError = (message, ...args) => console.error(`%c❌ [桃宝App] ${message}`, 'color: #c0392b;', ...args);
    ...
    const TaobaoApp = {
        ...
        injectTaobaoStyles: function() {
            ...
            // 原封不动地嵌入你的所有CSS，并用 #taobao-screen 进行隔离
            style.textContent = `
            ...
            htmlWrapper.innerHTML = `
                <div id="taobao-screen" class="screen" style="display:none; flex-direction:column;">
                    ...
                    <div class="taobao-content">
                        <div id="taobao-products-view" class="taobao-view active"><div class="taobao-search-bar"><input type="search" id="taobao-product-search-input" placeholder="搜一搜，让AI为你创造好物！"><button id="taobao-product-search-btn">搜索</button></div><div id="taobao-product-category-tabs"></div><div id="taobao-product-grid" class="product-grid"></div></div>
        ...
        bindEvents: function() {
            ...
            taobaoScreen.addEventListener('click', async (e) => {
                ...
                if (tab) { this.switchTaobaoView(tab.dataset.view); return; }
                
                if (target.id === 'taobao-add-product-btn') { this.openAddProductChoiceModal(); return; }
                if (target.id === 'taobao-clear-products-btn') { await this.clearTaobaoProducts(); return; }
                if (target.id === 'taobao-top-up-btn') { await this.handleTopUp(); return; }
                
                const increaseBtn = target.closest('.quantity-increase');
                if (increaseBtn) { const cartId = parseInt(increaseBtn.dataset.cartId); if (!isNaN(cartId)) await this.handleChangeCartItemQuantity(cartId, 1); return; }

                const decreaseBtn = target.closest('.quantity-decrease');
                if (decreaseBtn) { const cartId = parseInt(decreaseBtn.dataset.cartId); if (!isNaN(cartId)) await this.handleChangeCartItemQuantity(cartId, -1); return; }
                
                const deleteBtn = target.closest('.delete-cart-item-btn');
                if (deleteBtn) { const cartId = parseInt(deleteBtn.dataset.cartId); if (!isNaN(cartId)) { const c = await EPhone.showCustomConfirm('移出购物车', '确定要删除这个宝贝吗？'); if (c) await this.handleRemoveFromCart(cartId); } return; }
                
                const productCard = target.closest('.product-card, .cart-item');
                if (productCard && !target.closest('.add-cart-btn, .quantity-controls, .delete-cart-item-btn')) {
                    let productId = productCard.dataset.productId ? parseInt(productCard.dataset.productId) : null;
                    if (!productId) { // 兼容购物车内商品图片点击
                        const img = productCard.querySelector('img[data-product-id]');
                        if (img) productId = parseInt(img.dataset.productId);
                    }
                    if (!isNaN(productId)) await this.openProductDetail(productId);
                    return;
                }
                
                const addToCartBtn = target.closest('.add-cart-btn');
                if (addToCartBtn) { const productId = parseInt(addToCartBtn.dataset.productId); if (!isNaN(productId)) await this.handleAddToCart(productId); return; }

                const categoryBtn = target.closest('.category-tab-btn');
                if (categoryBtn) { const category = categoryBtn.dataset.category; this.state.currentTaobaoCategory = category; await this.renderTaobaoProducts(category === 'all' ? null : category); return; }
                
                if (target.id === 'taobao-checkout-btn') { await this.handleCheckout(); return; }
            });

            const bindModalAction = (btnId, handler) => {
                const btn = document.getElementById(btnId);
                if (btn) btn.addEventListener('click', handler);
                else logError(`绑定事件失败：找不到元素 #${btnId}`);
            };
            
            bindModalAction('taobao-add-product-manual-btn', () => { document.getElementById('taobao-add-product-choice-modal').classList.remove('visible'); this.openProductEditor(); });
            bindModalAction('taobao-cancel-add-choice-btn', () => document.getElementById('taobao-add-product-choice-modal').classList.remove('visible'));
            bindModalAction('taobao-save-product-btn', this.saveProduct.bind(this));
            bindModalAction('taobao-cancel-product-editor-btn', () => document.getElementById('taobao-product-editor-modal').classList.remove('visible'));
            bindModalAction('taobao-close-product-detail-btn', () => document.getElementById('taobao-product-detail-modal').classList.remove('visible'));
            document.getElementById('taobao-detail-add-to-cart-btn').addEventListener('click', async (e) => {
        ...
        open: async function() {
            logInfo("打开桃宝App");
            await this.renderTaobaoProducts(this.state.currentTaobaoCategory === 'all' ? null : this.state.currentTaobaoCategory);
            await this.renderBalanceDetails();
            await this.updateCartBadge();
        ...
        handleTopUp: async function() {
            try {
                ...
                await this.db.userWalletTransactions.add({ amount, description: '钱包充值', timestamp: Date.now() });
                logInfo(`本地交易记录已创建`);
                ...
                logError(`充值过程中发生错误:`, error);
                await window.EPhone.showCustomAlert('充值失败', `发生了一个错误: ${error.message}`);
            }
        },

        switchTaobaoView: async function(viewId) { /* ... implementation ... */ },
        renderBalanceDetails: async function() { /* ... implementation ... */ },
        renderWalletTransactions: async function() { /* ... implementation ... */ },
        updateCartBadge: async function() { /* ... implementation ... */ },
        handleAddToCart: async function(productId) { /* ... implementation ... */ },
        handleChangeCartItemQuantity: async function(cartId, change) { /* ... implementation ... */ },
        handleRemoveFromCart: async function(cartId) { /* ... implementation ... */ },
        openProductDetail: async function(productId) { /* ... implementation ... */ },
        renderTaobaoProducts: async function(category = null) { /* ... implementation ... */ },
        showProductActions: async function(productId) { /* ... implementation ... */ },
        openAddProductChoiceModal: function() { /* ... implementation ... */ },
        openProductEditor: async function(productId = null) { /* ... implementation ... */ },
        saveProduct: async function() { /* ... implementation ... */ },
        renderTaobaoCart: async function() { /* ... implementation ... */ },
        renderTaobaoOrders: async function() { /* ... implementation ... */ },
        handleCheckout: async function() { /* ... implementation ... */ },
        clearTaobaoProducts: async function() { /* ... implementation ... */ }
    };
    ...
    Object.assign(TaobaoApp, {
        ...
        renderBalanceDetails: async function() {
            ...
            if (!balanceDisplay) { logError("找不到余额显示元素 #taobao-user-balance-display"); return; }
            try {
        ...
        renderWalletTransactions: async function() {
            ...
            transactions.forEach(tx => {
                ...
                itemEl.innerHTML = `<div class="transaction-info"><div class="description">${tx.description}</div><div class="timestamp">${new Date(tx.timestamp).toLocaleString()}</div></div><div class="transaction-amount ${isIncome ? 'income' : 'expense'}">${isIncome ? '+' : ''} ¥${tx.amount.toFixed(2)}</div>`;
        ...
        updateCartBadge: async function() {
            ...
            } catch (error) { logError("更新购物车角标失败:", error); }
        },
        ...
        renderTaobaoProducts: async function(category = null) {
            ...
            gridEl.innerHTML = allProducts.map(p => `
                <div class="product-card" data-product-id="${p.id}">
                    <img src="${p.imageUrl}" class="product-image" alt="${p.name}" data-product-id="${p.id}">
        ...
        showProductActions: async function(productId) {
             const result = await EPhone.showCustomActionSheet([
                { text: '编辑商品', value: 'edit' },
                { text: '删除商品', value: 'delete', isDestructive: true },
             ]);

            if (result === 'edit') {
                this.openProductEditor(productId);
            } else if (result === 'delete') {
                const confirmed = await EPhone.showCustomConfirm('确认删除', '确定要删除这个商品吗？此操作无法恢复。');
                if (confirmed) {
                    await this.db.taobaoProducts.delete(productId);
                    await this.renderTaobaoProducts(this.state.currentTaobaoCategory === 'all' ? null : this.state.currentTaobaoCategory);
                    await EPhone.showCustomAlert("成功", "商品已删除");
                }
            }
        },
        ...
        saveProduct: async function() {
            ...
            if (!name || isNaN(price) || price < 0) { await window.EPhone.showCustomAlert('错误', '商品名称和有效的价格不能为空！'); return; }
        ...
        renderTaobaoOrders: async function(){
             ...
             if(listEl) listEl.innerHTML = '<p style="text-align:center; color: var(--text-secondary); padding: 50px 0;">你还没有任何订单哦~</p>';
        },
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            if (!TaobaoApp.isRunningInEPhone) {
                 ...
                 if(window.EPhone && typeof window.EPhone.showScreen === 'function') {
